...
-- Add user_id column to clients table as nullable first
...
-- For development: Delete existing clients without users (since we can't assign them properly)
-- In production, you'd want to assign them to actual users instead
DELETE FROM public.clients WHERE user_id IS NULL;

-- Now make user_id required for future inserts
