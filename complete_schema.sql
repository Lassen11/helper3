-- Полная схема базы данных для системы управления клиентами банкротства
-- Создано на основе миграций из репозитория

-- 1. Создание таблицы клиентов для банкротства
...
-- 8. Функция для обновления timestamp
...
-- 9. Функция для проверки роли пользователя
...
-- 10. Функция для проверки является ли пользователь админом
...
-- 11. Функция для автоматического создания профиля и роли пользователя
...
-- 12. Функция для создания первого админа
...
-- 13. Создание триггеров для автоматического обновления updated_at
...
-- 14. Создание триггера для автоматического создания профиля при регистрации
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
CREATE TRIGGER on_auth_user_created
  AFTER INSERT ON auth.users
  FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();

-- 15. Включение RLS для всех таблиц
...
-- 17. RLS политики для user_roles
...
-- 20. RLS политики для payment_receipts
...
-- 21. Создание storage bucket для чеков
INSERT INTO storage.buckets (id, name, public) VALUES ('receipts', 'receipts', false) ON CONFLICT (id) DO NOTHING;

-- 22. Storage политики для чеков
...
-- 23. Добавление комментариев для документации
...
-- 24. Создание первого админа из существующего пользователя, если админов еще нет
